...
const StudentClassesNew = () => {
  ...
  // 2. Fetch Classes when Course Selected
  useEffect(() => {
    const fetchClasses = async () => {
        ...
        try {
            setLoading(true);
            // Uses the same endpoint as coach, but backend validates student enrollment
            const response = await apiClient.get(ENDPOINTS.NEW_CLASSES.GET_BY_COURSE(selectedCourseId));
  ...
  const handleJoinClass = (roomId) => {
      navigate(`/student/classroom/${roomId}`);
  };
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            <div>
                <h1 className="text-3xl font-extrabold flex items-center gap-2">
                    <Video className="w-8 h-8 text-violet-500"/> My Live Classes
                </h1>
                <p className="text-gray-400 text-sm mt-1">Select a course to view your schedule</p>
            </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                ...
                {!selectedCourseId ? (
                    <div className="p-12 text-center bg-white/5 rounded-2xl border border-dashed border-gray-700 text-gray-400 flex flex-col items-center">
                        ...
                        Please select a course to view classes.
                    </div>
                ) : loading ? (
                    ...
                    <div className="p-8 text-center bg-white/5 rounded-xl border border-white/5 text-gray-400">
                        No upcoming classes scheduled for this course.
                    </div>
                ) : (
            ...
            {selectedCourseId && completedClasses.length > 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pt-8 border-t border-gray-800">
                    <h2 className="text-xl font-semibold flex items-center gap-2 text-green-500">
                        <CheckCircle className="w-5 h-5"/> Past Classes
                    </h2>
...
export default StudentClassesNew;